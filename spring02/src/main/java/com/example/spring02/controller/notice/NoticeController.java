package com.example.spring02.controller.notice;
...
import com.example.spring02.service.notice.NoticeService;
...
public class NoticeController {
	...
	public List<String> getAttach(@PathVariable("bno") int bno) {
		...
		List<Map<String, Object>> listA = noticeService.getAttach(bno);
	...
	public String delete(int bno) throws Exception {
		noticeService.delete(bno); // 삭제 처리
	...
	// 게시물 내용 수정
		...
		public String update(BoardDTO dto) throws Exception {
			...
			if(dto != null) {
				noticeService.update(dto); // 레코드 수정
	...
	public ModelAndView list(@RequestParam(defaultValue = "title") String search_option,
		...
		// 게시물 목록
		List<BoardDTO> list = noticeService.listAll(search_option, keyword, start, end);
	...
	public ModelAndView view(int bno, HttpSession session) throws Exception{
		...
		mav.setViewName("notice/view"); // 포워딩 할 뷰의 이름
		mav.addObject("dto", noticeService.read(bno)); // 자료저장
